...
function do_release(states_dict::Dict, params_dict::Dict)
    """Function to release a number of Wolbachia-infected mosquitoes into the tracked household."""
    
    @unpack K, rel_male, rel_fem = params_dict
    rel_size = rel_fem + rel_male  # total number of released mosquitoes
    household = states_dict["track"]
    H_ = household.num
    ...
    dist = event_ws / prop_i   # normalize the event weights to get the event distribution
    states_dict["track"] = Household_mut(fem_m, male_m, fem_w, male_w, H_, dist, prop_i, prop_i * H_, Categorical(dist)) 
