...
function do_release(states_dict::Dict, params_dict::Dict)
    ...
    states_dict_["free"] = states_dict["free"]       # add free state
    states_dict_["release"] = states_dict["release"]  # add release state
    ...
    for key in keys_list    # loop through the keys of the states dictionary
        ...
        if m_plus_w + rel_size > 2 * K   # if the household is full, sample mosquitoes to release
        ...
        else   # if household is not full, release all mosquitoes
            fem_w += rel_fem      # add females 
            male_w += rel_male    # add males 
        end
        # make a key for the household state after release
    ...
    return states_dict_   # return the updated states dictionary with the new household states after release
end
...
function initialise_gillespie(params_dict::Dict)
    ...
    @unpack fem_m0, male_m0, fem_w0, male_w0, H, m_free0, w_free0, rho, t_start, t_end, rel_t, delta_rel = params_dict  # unpack the parameter values
    ...
    male_w_vec, fem_w_vec, tOut, rel_t, delta_rel, num_house_vec, key_house_vec, track_fem_m, track_male_m, 
    track_fem_w, track_male_w 
end
...
function gillespie(params_dict::Dict)
    ...
    rel_t, delta_rel, num_house_vec, key_house_vec, track_fem_m, track_male_m, track_fem_w, track_male_w = initialise_gillespie(params_dict)
    ...
    return m_vec, w_vec, free_m_vec, free_w_vec, num_house_vec, key_house_vec, track_fem_m, track_male_m, track_fem_w, track_male_w
end
