...
rel_fem = parse(Int, ARGS[6])      # number of Wolbachia-infected female mosquitoes released into the community
rel_male = parse(Int, ARGS[7])     # number of Wolbachia-infected male mosquitoes released into the community
delta_rel = parse(Int, ARGS[8])    # time in days between releases
rel_prop = parse(Float64, ARGS[9])    # proportion of mosquitoes released into the community
...
# number of realisations of SSA to run, 10,000 is a good number for the paper
...
@everywhere module Wolbachia_coupled_para ## for household release need to change script
    include("SSA_reg_rel_propn_hhold.jl")  
end
...
# non-releasing tracked household
...
# releasing tracked household
track_fem_m_rel_results = SharedArray{Int}(result_length,num_simulations)
track_male_m_rel_results = SharedArray{Int}(result_length,num_simulations)
track_fem_w_rel_results = SharedArray{Int}(result_length,num_simulations)
track_male_w_rel_results = SharedArray{Int}(result_length,num_simulations)
...
@time begin
    @sync @distributed for i in 1:num_simulations
        ...
        does_rel_hholds = SharedArray{Int}(weeks, H) # stores whether household is releasing or not

        # Run the simulation
        # collecting the results from the simulation
        m_vec, w_vec, free_m_vec, free_w_vec, num_house_vec, key_house_vec, track_fem_m_rel, track_male_m_rel, track_fem_w_rel, track_male_w_rel = Wolbachia_coupled_para.gillespie(parameters)
        ...
        track_fem_m_rel_results[:, i] = track_fem_m_rel    # releasing tracked household no. female wildtypes each day, for sim i
        track_male_m_rel_results[:, i] = track_male_m_rel  # releasing tracked household no. male wildtypes each day, for sim i
        track_fem_w_rel_results[:, i] = track_fem_w_rel    # releasing tracked household no. female W-infected each day, for sim i
        track_male_w_rel_results[:, i] = track_male_w_rel  # releasing tracked household no. male W-infected each day, for sim i
        ...
        for t in 1:vec_length  # loop over each day
            ...
            for j in 1:length(key_house_vec[t])  # loop over number of household states occupied at time point
                ...
                if key_house_vec[t][j] != [999, 999, 999, 999, 999] && num > 0
                    ...
                    does_rel_hholds[t, num1:num2] .= key_house_vec[t][j][5] # whether household is releasing or not
        ...
        df_does_rel_hholds = DataFrame(does_rel_hholds, :auto)

        # Write to CSV files
        # the strings of values distinguish the different parameter sets
        CSV.write(joinpath(JOB_ID,"fem_m_hholds_$(i)_$(H)__$(rel_t)_$(rel_male)_$(rel_fem)_$(delta_rel)_relhhold.csv"), df_fem_m_hholds)
        CSV.write(joinpath(JOB_ID,"male_m_hholds_$(i)_$(H)__$(rel_t)_$(rel_male)_$(rel_fem)_$(delta_rel)_relhhold.csv"), df_male_m_hholds)
        CSV.write(joinpath(JOB_ID,"fem_w_hholds_$(i)_$(H)__$(rel_t)_$(rel_male)_$(rel_fem)_$(delta_rel)_relhhold.csv"), df_fem_w_hholds)
        CSV.write(joinpath(JOB_ID,"male_w_hholds_$(i)_$(H)__$(rel_t)_$(rel_male)_$(rel_fem)_$(delta_rel)_relhhold.csv"), df_male_w_hholds)
        CSV.write(joinpath(JOB_ID,"does_rel_hholds_$(i)_$(H)__$(rel_t)_$(rel_male)_$(rel_fem)_$(delta_rel)_relhhold.csv"), df_does_rel_hholds)
    end
end
...
df_track_fem_m_rel = DataFrame(track_fem_m_rel_results, :auto) # releasing tracked household female wildtypes each day each simulation
df_track_male_m_rel = DataFrame(track_male_m_rel_results, :auto) # releasing tracked household male wildtypes each day each simulation
df_track_fem_w_rel = DataFrame(track_fem_w_rel_results, :auto) # releasing tracked household female Wolbachia each day each simulation
df_track_male_w_rel = DataFrame(track_male_w_rel_results, :auto) # releasing tracked household male Wolbachia each day each simulation
...
CSV.write(joinpath(JOB_ID,"m_mosqs_$(H)__$(rel_t)_$(rel_male)_$(rel_fem)_$(delta_rel)_relhhold.csv"), df_m)
CSV.write(joinpath(JOB_ID,"w_mosqs_$(H)__$(rel_t)_$(rel_male)_$(rel_fem)_$(delta_rel)_relhhold.csv"), df_w)
# for total free wildtypes and Wolbachia-infected mosquitoes
CSV.write(joinpath(JOB_ID,"free_m_mosqs_$(H)__$(rel_t)_$(rel_male)_$(rel_fem)_$(delta_rel)_relhhold.csv"), df_free_m)
CSV.write(joinpath(JOB_ID,"free_w_mosqs_$(H)__$(rel_t)_$(rel_male)_$(rel_fem)_$(delta_rel)_relhhold.csv"), df_free_w)
# for tracked household mosquitoes of each type
#CSV.write(joinpath(JOB_ID,"track_fem_m_$(H)__$(rel_t)_$(rel_male)_$(rel_fem)_$(delta_rel)_relhhold.csv"), df_track_fem_m)
#CSV.write(joinpath(JOB_ID,"track_male_m_$(H)__$(rel_t)_$(rel_male)_$(rel_fem)_$(delta_rel)_relhhold.csv"), df_track_male_m)
#CSV.write(joinpath(JOB_ID,"track_fem_w_$(H)__$(rel_t)_$(rel_male)_$(rel_fem)_$(delta_rel)_relhhold.csv"), df_track_fem_w)
#CSV.write(joinpath(JOB_ID,"track_male_w_$(H)__$(rel_t)_$(rel_male)_$(rel_fem)_$(delta_rel)_relhhold.csv"), df_track_male_w)
# for releasing tracked household mosquitoes of each type
CSV.write(joinpath(JOB_ID,"track_fem_m_rel_$(H)__$(rel_t)_$(rel_male)_$(rel_fem)_$(delta_rel)_relhhold.csv"), df_track_fem_m_rel)
CSV.write(joinpath(JOB_ID,"track_male_m_rel_$(H)__$(rel_t)_$(rel_male)_$(rel_fem)_$(delta_rel)_relhhold.csv"), df_track_male_m_rel)
CSV.write(joinpath(JOB_ID,"track_fem_w_rel_$(H)__$(rel_t)_$(rel_male)_$(rel_fem)_$(delta_rel)_relhhold.csv"), df_track_fem_w_rel)
CSV.write(joinpath(JOB_ID,"track_male_w_rel_$(H)__$(rel_t)_$(rel_male)_$(rel_fem)_$(delta_rel)_relhhold.csv"), df_track_male_w_rel)
