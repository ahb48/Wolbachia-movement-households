...
function get_population_size(states_dict::Dict)
    ...
    release_fem_pop, release_male_pop = 0, 0  # initialise the total population sizes of released mosquitoes
...
function initialise_gillespie(params_dict::Dict)
    ...
    @unpack fem_m0, male_m0, fem_w0, male_w0, H, m_free0, w_free0, rho, t_start, t_end, delta_rel, rel_size, rel_t = params_dict  # unpack the parameter values
    ...
    # initialise female released mosquito vector
    release_fem_vec = zeros(Float64, len_t_vec)
    release_fem_vec[1] = 0.0  # initially 0
    # initialise male released mosquito vector
    release_male_vec = zeros(Float64, len_t_vec)
    release_male_vec[1] = 0.0  # initially 0
    ...
    delta_rel, rel_size, rel_t, num_house_vec, key_house_vec, track_fem_m, track_male_m, track_fem_w, track_male_w, release_fem_vec, release_male_vec 
end
...
function gillespie(params_dict::Dict)
    ...
    delta_rel, rel_size, rel_t, num_house_vec, key_house_vec, track_fem_m, track_male_m, track_fem_w, track_male_w, release_fem_vec, release_male_vec = initialise_gillespie(params_dict)
    ...
    while true  # Keeps iterating until break when passes final output time, or population goes extinct
        ...
        if t >= rel_t && rel_time # update the infected number of free mosquitoes after release
            # assume half release is male only
            rel_half = round(rel_size/2)
            update_number!(rel_half, "male_w", states_dict["release"], params_dict)
            update_number!(rel_half, "fem_w", states_dict["release"], params_dict)
        ...
        if t >= t_out                       # Check if past next time point            
            ...
            # record the number of released mosquitoes at the current time point
            release_fem_vec[i_out] = release_fem_pop
            release_male_vec[i_out] = release_male_pop
    ...
    return m_vec, w_vec, free_m_vec, free_w_vec, num_house_vec, key_house_vec, track_fem_m, track_male_m, track_fem_w, track_male_w, release_fem_vec, release_male_vec
end
