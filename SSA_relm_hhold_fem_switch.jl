...
function do_release(states_dict::Dict, params_dict::Dict)
    ...
    for key in keys_list    # loop through the keys of the states dictionary
        if (key != "free") && (key != "release")  # if the key is not the free or release state
            ...
            if m_plus_w + rel_size <= 2*K   # if the household can accommodate the release size
                male_w += rel_size   # add the release size to the number of Wolbachia-infected
            else  # if the household cannot accommodate the release size
                rel_ = round(Int, max(2*K - m_plus_w, 0))   # calculate the number of Wolbachia-infected mosquitoes that can be released
                male_w += rel_  # add the release size to the number of Wolbachia-infected
            end
            key_ = "track"   # if the household is the tracked household, set the key to "track"
            if key != "track"  # if the household is not the tracked household
                key_ = make_key_house(fem_m, male_m, fem_w, male_w)  # create a key for the household state
            end
    ...
    return states_dict_  # return the updated states dictionary after release
end
...
function initialise_gillespie(params_dict::Dict)
    ...
    delta_rel, rel_size, rel_t, num_house_vec, key_house_vec, track_fem_m, track_male_m, track_fem_w, track_male_w 
end
...
function gillespie(params_dict::Dict)
    ...
    delta_rel, rel_size, rel_t, num_house_vec, key_house_vec, track_fem_m, track_male_m, track_fem_w, track_male_w = initialise_gillespie(params_dict)
