...
@everywhere module Wolbachia_coupled_para ## for household release need to change script
    include("SSA_rel_com_fem_switch.jl")  
end
...
release_fem_results = SharedArray{Float64}(result_length,num_simulations) # stores number of released females each day
release_male_results = SharedArray{Float64}(result_length,num_simulations) # stores number of released males each day
...
@time begin
    @sync @distributed for i in 1:num_simulations
        ...
        track_fem_w, track_male_w, release_fem_vec, release_male_vec = Wolbachia_coupled_para.gillespie(parameters)
        ...
        release_fem_results[:, i] = release_fem_vec  # released females each day, for sim i
        release_male_results[:, i] = release_male_vec # released males each day, for sim i
...
df_release_fem = DataFrame(release_fem_results, :auto) # total released females each day each simulation
df_release_male = DataFrame(release_male_results, :auto) # total released males each day each simulation
...
# for total released Wolbachia-infected mosquitoes
CSV.write(joinpath(JOB_ID,"release_fem_$(alpha_str)_$(gamma_str)_$(rho_str)_$(tau_str)_$(H)__$(rel_t)_$(rel_size)_relhhold.csv"), df_release_fem)
CSV.write(joinpath(JOB_ID,"release_male_$(alpha_str)_$(gamma_str)_$(rho_str)_$(tau_str)_$(H)__$(rel_t)_$(rel_size)_relhhold.csv"), df_release_male)
